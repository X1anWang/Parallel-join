...
int ecall_scalable_oblivious_join(char *input_path, size_t len) {
    ...
    arr[length1+8].key = 0;
    arr[length1+9].key = 0;
    arr[length1+10].key = 0;
    arr[length1+11].key = 0;
    arr[length1+12].key = 0;
    arr[length1+13].key = 0;
    arr[length1+14].key = 0;
    arr[length1+15].key = 0;
    for (int i = length1 + 16; i < length1 + length2; i++) {
        arr[i].key = 1 + ((i - 1 - length1) % (32767));
        //strncpy(arr[i].data, strtok(NULL, "\n"), DATA_LENGTH);
        arr[i].table_0 = false;
    }
