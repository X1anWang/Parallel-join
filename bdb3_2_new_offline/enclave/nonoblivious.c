...
/* Compares elements by the tuple (key2, ORP ID). The check for the ORP ID must
 * always be run (it must be oblivious whether the comparison result is based on
 * the key2 or on the ORP ID), since we leak info on duplicate key2s otherwise. */
static int mergesort_comparator(const void *a_, const void *b_,
        void *aux UNUSED) {
    const elem_t *a = a_;
    const elem_t *b = b_;
    int comp_key2 = (a->key2 > b->key2) - (a->key2 < b->key2);
    int comp_orp_id = (a->orp_id > b->orp_id) - (a->orp_id < b->orp_id);
    return (comp_key2 << 1) + comp_orp_id;
}
