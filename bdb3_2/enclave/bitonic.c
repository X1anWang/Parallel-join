...
bool compare2D(elem_t* a, elem_t* b) {
    bool c;
    c = (o_strcmp(a->key, b->key) == -1) || (dimension2D && (o_strcmp(a->key, b->key) == 0) && (a->j_order < b->j_order));
    return c;
}
...
bool compare2D_(int a, int b) {
    bool c;
    c = (o_strcmp(arr[a].key, arr[b].key) == -1) || (dimension2D && (o_strcmp(arr[a].key, arr[b].key) == 0) && (arr[a].j_order < arr[b].j_order));
    return c;
}
