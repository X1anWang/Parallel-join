...
bool compare2D(elem_t* a, elem_t* b) {
    bool c;
    c = dimension2D? (a->sum_adrevenue < b->sum_adrevenue) : (o_strcmp(a->key, b->key) == -1);
    return c;
}
...
bool compare2D_(int a, int b) {
    bool c;
    c = dimension2D? (arr[a].sum_adrevenue < arr[b].sum_adrevenue) : (o_strcmp(arr[a].key, arr[b].key) == -1);
    return c;
}
