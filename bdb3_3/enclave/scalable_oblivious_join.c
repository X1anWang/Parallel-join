...
struct tree_node_op2 {
    volatile char key_first[16];
    volatile char key_last[16];
...
void aggregation_tree_op2(void *voidargs) {
    ...
    for (int i = index_thread_start + 1; i < index_thread_end; i++) {
        condition = o_strcmp(arr[i].key, arr[i - 1].key) == 0;
        arr[i].sum_adrevenue = condition * arr[i - 1].sum_adrevenue + arr[i].sum_adrevenue;
        arr[i].avg_pagerank = condition * arr[i - 1].avg_pagerank + arr[i].avg_pagerank;
    ...
    o_strcmp_(ag_tree[cur_tree_node].key_first, arr[index_thread_start].key);
    o_strcmp_(ag_tree[cur_tree_node].key_last, arr[index_thread_end - 1].key);
    ...
    while(cur_tree_node % 2 == 0 && 0 < cur_tree_node) {
        ...
        condition = o_strcmp_(ag_tree[cur_tree_node - 1].key_last, ag_tree[cur_tree_node].key_last) == 0;
        condition1 = o_strcmp_(ag_tree[cur_tree_node - 1].key_first, ag_tree[cur_tree_node].key_first) == 0;
        o_strcmp_(ag_tree[temp].key_first, ag_tree[cur_tree_node - 1].key_first);
        o_strcmp_(ag_tree[temp].key_last, ag_tree[cur_tree_node].key_last);
    ...
    while(cur_tree_node < thread_order) {
        ...
        condition = o_strcmp_(ag_tree[temp1].key_last, ag_tree[temp].key_first) == 0;
        condition1 = o_strcmp_(ag_tree[cur_tree_node].key_first, ag_tree[temp].key_first) == 0;
        condition2 = o_strcmp_(ag_tree[cur_tree_node].key_last, ag_tree[temp1].key_last) == 0;
    ...
    char key_first[16];
    char key_last[16];
    o_strcp_(key_first, arr[index_thread_start].key);
    int sum_previous = ag_tree[thread_order].sum_prefix;
    o_strcp_(key_last, arr[index_thread_end - 1].key);
    int sum_last = ag_tree[thread_order].sum_suffix;
    int sum_previous2 = ag_tree[thread_order].sum_prefix2;
    int sum_last2 = ag_tree[thread_order].sum_suffix2;
    int sum_previous3 = ag_tree[thread_order].sum_prefix3;
    int sum_last3 = ag_tree[thread_order].sum_suffix3;
    //printf("\nCheck 5, from thread %d\n", thread_order);
    for (int i = index_thread_start; i < index_thread_end; i++) {
        condition = o_strcmp(arr[i].key, key_first) == 0;
        condition1 = o_strcmp(arr[i].key, key_last) == 0;
