...
static sgx_status_t SGX_CDECL Enclave_ocall_init_time(void* pms)
{
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
	ocall_init_time();
	return SGX_SUCCESS;
...
static sgx_status_t SGX_CDECL Enclave_ocall_init_comp_clock(void* pms)
{
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
...
static sgx_status_t SGX_CDECL Enclave_ocall_get_comp_time(void* pms)
{
	if (pms != NULL) return SGX_ERROR_INVALID_PARAMETER;
	ocall_get_comp_time();
	return SGX_SUCCESS;
...
static const struct {
	...
	18,
	{
		...
		(void*)Enclave_ocall_init_time,
		(void*)Enclave_ocall_get_time,
		(void*)Enclave_ocall_init_comp_clock,
		(void*)Enclave_ocall_get_comp_time,
...
{
	sgx_status_t status;
	ms_branching_factor_test_t ms;
	ms.ms_maxsize = maxsize;
