...
int ecall_scalable_oblivious_join(char *input_path, size_t len) {
    ...
    printf("\nLength1 is %lld", length1);
    printf("\nLength2 is %lld", length2);
    long long num = 0;
    long long total = length1 + length2;
    long long c = total * 100 / 99;
    long long index_1 = 0;
    long long index_2 = length1;
    long long sum = 0;
    long long num1 = 0;
    long long num2 = 0;
    for (long long ii = 1; ii <= length1 && sum < length1; ii++) {
        num = (length1 - sum) < (c * 1 / (ii * ii)) ? (length1 - sum) : (c * 1 / (ii * ii));
        num1 = (num % 2 == 0) ? 2 : 1;
        num2 = num / num1;
        for(int u = 0; u < num1; u++) {
            arr[index_1].key = ii;
            arr[index_1++].table_0 = true;
        }
        for(int u = 0; u < num2; u++) {
            arr[index_2].key = ii;
            arr[index_2++].table_0 = false;
        }
        sum += num;
    }

    for (; index_1 < length1; ) {
        arr[index_1].key = total + 1;
        arr[index_1++].table_0 = true;
    }

    for (; index_2 < length1 + length2; ) {
        arr[index_2].key = total + 2;
        arr[index_2++].table_0 = false;
    }
