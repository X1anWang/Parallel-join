...
static long long get_index_address(size_t index) {
    return index * world_size / total_length;
}

static size_t get_local_start(long long rank) {
...
static void swap_remote_range(void *args_, size_t thread_idx) {
    ...
    size_t local_start = get_local_start(world_rank);
    long long remote_rank = get_index_address(remote_idx);
...
void bitonic_merge(void *voidargs) {
    ...
    long long number_threads = args->number_threads;
    ...
    if (1 < number_threads) {
        long long number_threads_left = number_threads / 2;
        long long number_threads_right = number_threads - number_threads_left;
...
void bitonic_sort_(elem_t *arr_, bool ascend, long long lo, long long hi, long long number_threads, bool D2enable) {
    arr = arr_;
    dimension2D = D2enable;
