...
void scalable_oblivious_join(elem_t *arr, int length1, int length2, char* output_path){
    ...
    for (int i = 1; i < number_threads; i++) {
        condition = (arr[args_op2_[i].index_thread_start - 1].key == arr[args_op2_[i].index_thread_start].key);
        condition2 = (arr[args_op2_[i].index_thread_start - 1].key == arr[args_op2_[i].index_thread_end - 1].key);
        sum[args_op2_[i].index_thread_start] += condition * sum[args_op2_[i].index_thread_start - 1];
        sum[args_op2_[i].index_thread_end - 1] += condition2 * sum[args_op2_[i].index_thread_start - 1];
    }
    get_time(true);
    for (int i = 0; i < number_threads - 1; i++) {
