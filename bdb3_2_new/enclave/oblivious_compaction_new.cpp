...
void oswap_range_elem(void *voidargs) {
    ...
    uint64_t block_size = args->block_size;
    uint64_t swap_start = args->swap_start;
    uint64_t swap_end = args->swap_end;
    uint64_t offset_m1_mod = args->offset_m1_mod;
    ...
    for(uint64_t i=swap_start; i<swap_end; i++){
...
void oblivious_compact_2power_inner_elem(elem_t *buf, uint64_t N, uint64_t block_size, uint64_t offset, bool *selected, uint64_t*selected_count) {
  ...
  uint64_t offset_mod = (offset & ((N/2)-1));
  uint64_t offset_m1_mod = (offset+m1) & ((N/2)-1);
  ...
  if (true) {
    ...
    uint64_t num_swap = N/2;
    for(uint64_t i=0; i<num_swap; i++){
...
void oblivious_compact_inner_elem(elem_t *buf, uint64_t N, uint64_t block_size, bool *selected, uint64_t*selected_count) {
  if (N==0) {
    return ;
  }
  else if (N==1) {
  ...
  if (true) {
    ...
    mL = 0;
    for(uint64_t i=0; i<split_index; i++){
      mL+=selected[i];
    }
  }
...
void oblivious_compact_inner_2power_parallel_elem(void *args_) {
  ...
  uint64_t N = args0->length;
  uint64_t block_size = args0->block_size;
  uint64_t offset = args0->offset;
  bool *selected = args0->selected;
  uint64_t *selected_count = args0->selected_count;
  uint64_t nthreads = args0->nthreads;
  ...
  bool offset_right = (offset >= N/2);
  bool left_wrapped = ((offset_mod + m1) >= (N/2));
  uint64_t lthreads = nthreads/2;
  uint64_t rthreads = nthreads - lthreads;
  ...
  uint64_t inc = num_swap / nthreads;
  uint64_t extra = num_swap % nthreads;
  uint64_t last = 0;
  for (uint64_t i=0; i<nthreads; ++i) {
    uint64_t next = last + inc + (i < extra);
...
void oblivious_compact_inner_parallel_elem(elem_t *buf, uint64_t N, uint64_t block_size, bool *selected, uint64_t *selected_count, uint64_t nthreads) {
...
void oblivious_compact_elem(elem_t *buf, bool *selected, uint64_t length, uint64_t block_size, uint64_t number_threads) {
    
    uint64_t *selected_count = NULL;
