...
static long long *index_target;
static long long *index_target2;
...
struct soj_scan_2_args {
    ...
    elem_t *arr1_;
    elem_t *arr2_;
    long long *index_target1;
    long long *index_target2;
    long long *index_target1_;
    long long *index_target2_;
    int order;
};
...
void soj_scan_1(void *voidargs) {
    ...
    long long index_start = args->idx_st;
    long long index_end = args->idx_ed;
    bool* cb1 = args->control_bit;
    bool* cb2 = args->control_bit_;
    elem_t *arr1 = args->arr1;
    elem_t *arr2 = args->arr2;

    for (long long i = index_start; i < index_end; i++) {
...
void soj_scan_2(void *voidargs) {
    ...
    long long index_start = args->idx_st;
    long long index_end = args->idx_ed;
    long long length1 = args->length1;
    long long length2 = args->length2;
    elem_t *arr1 = args->arr1;
    elem_t *arr2 = args->arr2;
    elem_t *arr1_ = args->arr1_;
    elem_t *arr2_ = args->arr2_;
    long long *index_target1 = args->index_target1;
    long long *index_target2 = args->index_target2;
    long long *index_target1_ = args->index_target1_;
    long long *index_target2_ = args->index_target2_;
...
void scalable_oblivious_join(elem_t *arr, long long length1, long long length2, char* output_path){
    ...
    printf("\nInput length: %lld and %lld", length1, length2);
    ...
    long long index_start_thread[number_threads + 1];
    ...
    long long length_result;
    index_target = calloc(length1, sizeof(*index_target));
    index_target2 = calloc(length2, sizeof(*index_target2));

    oe_mallinfo_t info2;
    oe_result_t rc2 = oe_allocator_mallinfo(&info2);
    printf("\n1 Heap size current is: %ld\n", info2.current_allocated_heap_size);
    printf("\n1 Heap size peak is: %ld\n", info2.peak_allocated_heap_size);
    (void)rc2;

    elem_t *arr1 = malloc(MAX_OUTPUT_LENGTH * sizeof(*arr1));
    elem_t *arr2 = malloc(MAX_OUTPUT_LENGTH * sizeof(*arr2));
    long long *index_target_ = malloc(MAX_OUTPUT_LENGTH * sizeof(*index_target_));
    long long *index_target2_ = malloc(MAX_OUTPUT_LENGTH * sizeof(*index_target2_));



    for (long long i = 0; i < MAX_OUTPUT_LENGTH; i++) {
        arr1[i].key = rand() % 1000;
        arr2[i].key = rand() % 1000;
        index_target_[i] = rand() % 1000;
        index_target2_[i] = rand() % 1000;
    }
    
    oe_mallinfo_t info4;
    oe_result_t rc4 = oe_allocator_mallinfo(&info4);
    printf("\n3 Heap size current is: %ld\n", info4.current_allocated_heap_size);
    printf("\n3 Heap size peak is: %ld\n", info4.peak_allocated_heap_size);
    (void)rc4;
    ...
    if (1 < number_threads) {
        ...
        // Upward scan
        for (long long i = length - 1; 0 < i; i--) {
    ...
    */
    ...
    get_time(true);
    oe_mallinfo_t info3;
    oe_result_t rc3 = oe_allocator_mallinfo(&info3);
    printf("\n2 Heap size current is: %ld\n", info3.current_allocated_heap_size);
    printf("\n2 Heap size peak is: %ld\n", info3.peak_allocated_heap_size);
    (void)rc3;
    ...
    oe_mallinfo_t info5;
    oe_result_t rc5 = oe_allocator_mallinfo(&info5);
    printf("\n5 Heap size current is: %ld\n", info5.current_allocated_heap_size);
    printf("\n5 Heap size peak is: %ld\n", info5.peak_allocated_heap_size);
    (void)rc5;
    get_time(true);
    //printf("\nStep 3 - completed");
    /*
    ...
    */
    ...
    if (1 < number_threads) {
        ...
        for (long long i = 1; i < length_result; i++) {
    ...
    */
    ...
    get_time2(true);    
    printf("\nOutput length is: %lld", length_result);
    // Write out
