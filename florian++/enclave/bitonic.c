...
bool compare2D_(int a, int b) {
    bool c;
    if (compareTID) {
        c = ((1 - arr[a].table_0) < (1 - arr[b].table_0)) || (((1 - arr[a].table_0) == (1 - arr[b].table_0)) && (arr[a].key < arr[b].key));
    } else {
...
void bitonic_sort_(elem_t *arr_, bool ascend, int lo, int hi, int number_threads, bool D2enable, bool TIDenable) {
    arr = arr_;
    dimension2D = D2enable;
    compareTID = TIDenable;
