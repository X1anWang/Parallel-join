...
void aggregation_tree_init(int nthreads) {
    ...
    tree_node_2[0].prev_elem->has_value = false;
...
void aggregation_tree_dup_1_downward(void *voidargs) {
    ...
    while(cur_tree_node % 2 == 0 && 0 < cur_tree_node) {
        ...
        condition = tree_node_2[cur_tree_node].last_elem->has_value;
    ...
    for (int i = idx_start; i < idx_end; i++) {
        o_memcpy(&arr[i], tree_node_2[order_thread].prev_elem, sizeof(*arr), !arr[i].has_value);
    }
