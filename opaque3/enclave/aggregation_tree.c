...
void o_strcp_(volatile char* str1, volatile char* str2, bool condition) {
    for (int i = 0; i < 105; i++) {
        str1[i] = condition * str2[i] + !condition * str1[i];
    }
}

void o_strcp(volatile char* str1,volatile char* str2) {
    o_strcp_(str1, str2, true);
}
...
struct tree_node_dup {
    ...
    volatile int prev_elem;
    volatile int first_elem;
    volatile int last_elem;
    elem_t prev_element;
    elem_t last_element;
    */
    volatile char k_p[105];
    volatile char d_p[DATA_LENGTH];
    volatile bool has_value_p;

    volatile char k_l[105];
...
void aggregation_tree_m_1_downward(void *voidargs) {
    ...
    for (int i = idx_start + 1; i < idx_end; i++) {
        condition = (o_strcmp_(arr[i].key, arr[i - 1].key) == 0);
    ...
    o_strcp(tree_node_1[cur_tree_node].key_first, arr[idx_start].key);
    o_strcp(tree_node_1[cur_tree_node].key_last, arr[idx_end - 1].key);
    ...
    while(cur_tree_node % 2 == 0 && 0 < cur_tree_node) {
        ...
        condition = o_strcmp_(tree_node_1[cur_tree_node - 1].key_last, tree_node_1[cur_tree_node].key_last) == 0;
        condition1 = o_strcmp_(tree_node_1[cur_tree_node - 1].key_first, tree_node_1[cur_tree_node].key_first) == 0;
        o_strcp(tree_node_1[temp].key_first, tree_node_1[cur_tree_node - 1].key_first);
        o_strcp(tree_node_1[temp].key_last, tree_node_1[cur_tree_node].key_last);
    ...
    while(cur_tree_node < order_thread) {
        ...
        condition = o_strcmp_(tree_node_1[temp1].key_last, tree_node_1[temp].key_first) == 0;
        condition1 = o_strcmp_(tree_node_1[cur_tree_node].key_first, tree_node_1[temp].key_first) == 0;
        condition2 = o_strcmp_(tree_node_1[cur_tree_node].key_last, tree_node_1[temp1].key_last) == 0;
...
void aggregation_tree_j_downward(void *voidargs) {
    ...
    j_index[idx_start] = 0;
    for (int i = idx_start + 1; i < idx_end; i++) {
        j_index[i] = (o_strcmp_(arr[i].key, arr[i - 1].key) == 0) * (j_index[i - 1] + 1);
    }
    ...
    o_strcp(tree_node_3[cur_tree_node].key, arr[idx_end - 1].key);
    ...
    while(cur_tree_node % 2 == 0 && 0 < cur_tree_node) {
        ...
        o_strcp(tree_node_3[temp].key, tree_node_3[cur_tree_node].key);
        tree_node_3[temp].j = tree_node_3[cur_tree_node].j + (o_strcmp_(tree_node_3[cur_tree_node].key, tree_node_3[cur_tree_node - 1].key) == 0) * tree_node_3[cur_tree_node - 1].j;
    ...
    if (idx_start != 0) {
        for (int i = idx_start; i < idx_end; i++) {
            j_index[i] = j_index[i] + (o_strcmp_(arr[i].key, arr[idx_start - 1].key) == 0) * tree_node_3[order_thread].prev_j;
        }
    } 
...
void aggregation_tree_dup_1_downward(void *voidargs) {
    ...
    o_strcp(tree_node_2[cur_tree_node].k_l, arr[idx_end - 1].key);
    ...
    */
    ...
    while(cur_tree_node % 2 == 0 && 0 < cur_tree_node) {
        ...
        */
        o_strcp_(tree_node_2[temp].k_l, tree_node_2[cur_tree_node].k_l, condition);
        o_strcp_(tree_node_2[temp].k_l, tree_node_2[cur_tree_node - 1].k_l, !condition);
    ...
    while(cur_tree_node < order_thread) {
        ...
        */
        ...
        o_strcp(tree_node_2[temp1].k_p, tree_node_2[cur_tree_node].k_p);
        ...
        o_strcp_(tree_node_2[temp].k_p, tree_node_2[temp1].k_l, condition);
        o_strcp_(tree_node_2[temp].k_p, tree_node_2[cur_tree_node].k_p, !condition);
    ...
    o_strcp(previous_element->key, tree_node_2[order_thread].k_p);
...
void aggregation_tree_m(elem_t *arr, int length, int number_threads) {
    ...
    for (int i = 0; i < number_threads; i++) {
        ...
        printf("end index: %d", idx_start_thread[i+1]);
        */
    ...
    /*
    for (int i = 0; i < 2 * number_threads - 1; i++) {
        printf("\n ----- m tree node : %d", i);
