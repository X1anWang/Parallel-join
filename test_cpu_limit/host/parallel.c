...
fclose(input_file);
    ...
    /* Check array. */
#ifndef DISTRIBUTED_SGX_SORT_HOSTONLY
    result = ecall_verify_joined(enclave, &ret);
    if (result != OE_OK) {
        handle_oe_error(result, "ecall_verify_joined");
        goto exit_free_arr;
    }
#else /* DISTRIBUTED_SGX_SORT_HOSTONLY */
    ret = ecall_verify_joined(world_rank);
#endif /* DISTRIBUTED_SGX_SORT_HOSTONLY */
    if (ret) {
        handle_error_string("Error verifying join");
        goto exit_free_arr;
    }
...
/*void cpu_computations_limit(long long size) {
    ...
    for(long long i = 0; i < size; i++) {
        arr[i].key = 8;
        for( size_t u = 0; u < DATA_LENGTH; u++)
            arr[i].data[u] = 7;
    }
    ...
    for(long long c = 0; c < size; c++) {
        o_memswap(&arr[c], &arr[(c+half) % size],1);
    }
    get_time(true);
...
int main(int argc, char **argv) {
...
#ifndef DISTRIBUTED_SGX_SORT_HOSTONLY
    if (argc < 4) {
        printf("usage: %s enclave_image array_size num_threads block_size\n", argv[0]);
#else /* DISTRIBUTED_SGX_SORT_HOSTONLY */
    if (argc < 3) {
    ...
    long long size = 1 << strtoll(argv[2], NULL, 10);
    printf("total arraysize: %lld\n", size);
	// SIZE = strtoll(argv[4], NULL, 10);
    errno = 0;
#ifndef DISTRIBUTED_SGX_SORT_HOSTONLY
	...
	*/
        ...
        printf("Executing experiments on enclave\n");
        ecall_cpu_computations_limit(enclave, &size);
#else
    ...
    //printf("Executing experiments on host\n");
    //cpu_computations_limit(size);
