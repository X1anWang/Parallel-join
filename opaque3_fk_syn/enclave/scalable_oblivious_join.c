...
void scalable_oblivious_join(elem_t *arr, int length1, int length2, char* output_path){
    ...
    for (int i = 1; i < number_threads; i++) {
        condition = !arr[args_op2_[i].index_thread_start].table_0;
        o_memcpy(arr_ + args_op2_[i].index_thread_start, arr + args_op2_[i].index_thread_start - 1, sizeof(*arr_), condition);
        condition = !arr[args_op2_[i].index_thread_end - 1].table_0;
        o_memcpy(arr_ + args_op2_[i].index_thread_end - 1, arr + args_op2_[i].index_thread_start - 1, sizeof(*arr_), condition); 
    }
    get_time(true);
    for (int i = 0; i < number_threads - 1; i++) {
